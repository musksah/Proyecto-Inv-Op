...
function encontrarMayores($data)
{
    ...
    $datost .= "<td><h4><center>Alternatives Decision</center></h4></td>";
    foreach ($headers as $key => $value) {
        $datost .= ' <th scope="col" class="text-center"><h4>' . $value . '</h4> </th>';
    }
    $datost .= ' <th scope="col" class="text-center"><h4>Minimum</h4></th>';
...
function PayOffMatrix($data)
{
    ...
    $("#form_payoff_data").submit(function (event) {
        ...
        $.ajax({
            ...
            let cadena = "";
            $.each(data.minimos, function (keyarr, value) {
                cadena += "<p>" + value + "</p>";
            });
            $("#minimo").html(cadena); 
